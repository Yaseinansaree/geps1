...
class Bot:
	...
	def getChatsUpdate(self,):
		return loads(self.enc.decrypt(post(json={"api_version":"5","auth": self.auth,"data_enc":self.enc.encrypt(dumps({ "method":"getChatsUpdates", "input":{ "state": str(round(datetime.today().timestamp()) - 1000), }, "client": Clients.web }))},url='https://messengerg2c64.iranlms.ir/').json().get("data_enc"))).get('data').get('chats')
